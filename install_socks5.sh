...
echo "=== Dante SOCKS5 一键部署脚本（自动兼容容器/NAMESPACE）==="
...
PUBLIC_IP_OVERRIDE="${PUBLIC_IP:-}"  # optional: override detected public ip
EXTERNAL_PORT="${EXTERNAL_PORT:-}"   # optional: nat external port for final url display
...
if [[ "$MODE" == "auth" ]]; then
  ...
  echo "[2/5] 无密码模式：跳过创建用户"
fi

# --------- write config ----------
echo "[3/5] 写入配置文件 /etc/danted.conf"
if [[ "$MODE" == "auth" ]]; then
...
# --------- start strategy ----------
echo "[4/5] 启动 danted（自动兼容 systemd NAMESPACE 限制）"
...
# If systemd failed, fallback to nohup (common in LXC/containers)
if [[ "$start_ok" -ne 1 ]]; then
  # stop any leftover
  pkill -f "/usr/sbin/danted.*-f /etc/danted.conf" 2>/dev/null || true
...
if [[ "$MACHINE" == "nat" ]]; then
  ...
  if [[ -z "$EXTERNAL_PORT" ]]; then
    echo "提示：你可以设置环境变量 EXTERNAL_PORT=外部端口，这样脚本会直接输出最终可用的一行地址。"
  fi
fi
